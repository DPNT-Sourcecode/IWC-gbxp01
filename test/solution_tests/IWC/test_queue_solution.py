...
def test_rule_of_3_with_timestamps() -> None:
    run_queue(
        [
            ...
            call_enqueue("bank_statements", 1, iso_ts(delta_minutes=3)).expect(2),
            ...
            call_enqueue("bank_statements", 2, iso_ts(delta_minutes=6)).expect(6),
...
def test_time_sensitive_bank_statements_exactly_5_minutes() -> None:
    run_queue(
        [
            call_enqueue("bank_statements", 1, iso_ts(delta_minutes=0)).expect(1),
...
def test_same_timestamp_bank_statements_and_companies_house() -> None:
    run_queue(
        [
            call_enqueue("companies_house", 1, iso_ts(delta_minutes=0)).expect(1),
